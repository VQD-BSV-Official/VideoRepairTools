#!/usr/bin/env python3
"""
h264_fixer.py  v5  -  H.264 / Sony XAVC video recovery tool
Sửa các lỗi thực tế từ log ffplay:
  - non-existing SPS 0 referenced in buffering period
  - cabac_init_idc 32 overflow  (SPS garbage lọt qua)
  - decode_slice_header error
  - SEI type 1/0/3/5/6/... truncated
  - Picture timing SEI payload too large
  - Invalid NAL unit 0/1/5, skipping

Cải tiến v5 (fix log out.h264 - file đầu ra vẫn còn lỗi):
  filter_sei bugfix   : psize so sanh voi RBSP available (sau strip emulation-prev)
                        thay vi raw byte count - day la bug chinh khien SEI truncated
                        van lọt vao output
  pic_timing too large: Drop SEI type 1 khi psize > MAX_PIC_TIMING (64 byte cho 1080p)
                        -> "Picture timing SEI payload too large"
  cabac overflow      : parse_sps kiem tra them entropy_coding_mode_flag va
                        num_ref_frames range de loai SPS garbage
  SEI type 5 truncated: user_data_unregistered (type 5) cung bi truncate -> ap dung
                        cung quy tac truncate check nhu cac type khac
  NAL type 5 invalid  : "Invalid NAL unit 5" -> validate_slice_header nghiem ngat hon,
                        kiem tra pps_id reference hop le
  Standard SEI whitelist mo rong: type 2,3,6,11,15,16,23,27 la standard nhung hiem gap
                        trong XAVC -> drop neu truncated, giu neu day du
"""
...
# SEI payload type whitelist (ITU-T H.264 Annex D, standard types only)
# Type >= 32 rat hiem dung va thuong la garbage trong file hong
SEI_TYPE_WHITELIST = set(range(32))          # 0-31 la standard
MAX_SEI_PAYLOAD    = 4096                    # >4096 byte 1 SEI payload = suspect
# SEI type 1 (pic_timing): theo spec H.264 E.2.2, toi da ~300 byte cho 1080p/HRD day du
# ffmpeg bao "Picture timing SEI payload too large" khi > 264 byte
MAX_PIC_TIMING     = 264
MIN_AU_BYTES       = 32                      # AU < 32 byte -> "missing picture" garbage
...
class SPSInfo:
    ...
    def plausible(self) -> bool:
        ...
        if not (0  <= self.num_ref_frames <= 16): return False
        return True
...
def parse_sps(nal_data: bytes) -> Optional[SPSInfo]:
    try:
        ...
        info = SPSInfo(sps_id, profile_idc, level_idc, w, h, num_ref_frames)
...
def validate_slice_header(nal_data: bytes) -> bool:
    """
    Kiem tra slice header co hop le khong.
    - first_mb_in_slice: phai trong pham vi MB cua frame 4K
    - slice_type:        0-9 (mod5 = 0:P 1:B 2:I 3:SP 4:SI)
    - pic_parameter_set_id: 0-255
    - IDR NAL (type 5):  nal_ref_idc phai != 0
    Log: "Invalid NAL unit 1/5, skipping" / "decode_slice_header error"
    """
    try:
        nal_hdr    = nal_data[0]
        nal_type   = nal_hdr & 0x1F
        nal_ref    = (nal_hdr >> 5) & 0x3
        # IDR slice phai co nal_ref_idc != 0 (quan trong)
        if nal_type == 5 and nal_ref == 0:
            return False
        br = BitReader(nal_data[1:])
        first_mb   = br.read_ue()
        ...
        if pps_id     > 255:   return False   # pps_id out of range
        return True
    except Exception:
        return False
...
def _rbsp_size(raw: bytes, start: int) -> int:
    """
    Dem so byte RBSP available tu vi tri start trong raw bytes.
    Can biet vi RBSP da bo emulation prevention bytes (0x000003),
    nen psize (RBSP) nho hon raw byte count.
    Vi du: raw=[..., 0x00, 0x00, 0x03, 0x01, ...] = 4 raw bytes = 3 RBSP bytes
    """
    count = 0
    i = start
    n = len(raw)
    while i < n:
        if i + 2 < n and raw[i] == 0 and raw[i+1] == 0 and raw[i+2] == 3:
            count += 2; i += 3   # byte 0x03 la emulation prevention, khong tinh
        else:
            count += 1; i += 1
    return count
...
def filter_sei(nal_data: bytes, valid_sps_ids: set) -> Optional[bytes]:
    """
    Duyet tung SEI payload, loc cac truong hop:
      1. ptype ngoai SEI_TYPE_WHITELIST (>= 32, non-standard / garbage)
         -> "SEI type 236/227/500 size NNN truncated at NNN"
      2. psize > RBSP bytes con lai trong NAL (truncated payload) [BUG FIX v5]
         Truoc day so sanh voi raw byte count -> sai khi co emulation prevention
         -> "SEI type 1 size 72 truncated at 64"
      3. SEI type 1 (pic_timing) psize > MAX_PIC_TIMING
         -> "Picture timing SEI payload too large"
      4. psize > MAX_SEI_PAYLOAD (payload bat thuong lon)
      5. buffering_period (type 0) tham chieu SPS id ngoai whitelist
         -> "SEI type 0 overread by N bits"
      6. psize == 0 (payload rong)
    Tra ve None neu khong con payload hop le nao.
    """
    if len(nal_data) < 2:
        return None
    nal_hdr = nal_data[0:1]
    # raw_sei: SODB bytes sau NAL header, chua bao gom emulation prevention removal
    raw_sei = nal_data[1:]
    i       = 0
    ...
    while i < len(raw_sei):
        type_start = i
        # Doc ptype (FF-encoded)
        ptype = 0
        while i < len(raw_sei) and raw_sei[i] == 0xFF:
            ptype += 255; i += 1
        if i >= len(raw_sei): break
        ptype += raw_sei[i]; i += 1

        # Doc psize (FF-encoded)
        psize = 0
        while i < len(raw_sei) and raw_sei[i] == 0xFF:
            psize += 255; i += 1
        if i >= len(raw_sei): break
        ...
        payload_raw_start = i   # vi tri bat dau payload trong raw_sei

        # So byte RBSP thuc su con lai tu payload_raw_start tro di
        rbsp_avail = _rbsp_size(raw_sei, payload_raw_start)

        # --- Loc 1: ptype non-standard (>= 32) ---
        if ptype not in SEI_TYPE_WHITELIST:
            log.debug(f"SEI drop: type {ptype} non-standard")
            i += min(psize, len(raw_sei) - payload_raw_start)
            dropped += 1; continue

        # --- Loc 2: truncated (psize > RBSP available) ---
        if psize == 0:
            dropped += 1; continue
        if psize > rbsp_avail:
            log.debug(f"SEI drop: type {ptype} psize={psize} > rbsp_avail={rbsp_avail} (truncated)")
            i += len(raw_sei) - payload_raw_start   # skip den cuoi
            dropped += 1; continue

        # --- Loc 3: pic_timing (type 1) qua lon ---
        if ptype == 1 and psize > MAX_PIC_TIMING:
            log.debug(f"SEI drop: pic_timing psize={psize} > MAX_PIC_TIMING={MAX_PIC_TIMING}")
            i += psize; dropped += 1; continue

        # --- Loc 4: oversized payload ---
        if psize > MAX_SEI_PAYLOAD:
            log.debug(f"SEI drop: type {ptype} psize={psize} > MAX_SEI_PAYLOAD={MAX_SEI_PAYLOAD}")
            i += psize; dropped += 1; continue

        # Lay raw bytes cua payload (can giu emulation prevention de re-encode dung)
        payload_raw = raw_sei[payload_raw_start:payload_raw_start + psize]
        i += psize

        # --- Loc 5: buffering_period kiem tra SPS id ---
        keep = True
        if ptype == 0:
            try:
                br2   = BitReader(payload_raw)
                bp_id = br2.read_ue()
                if bp_id not in valid_sps_ids:
                    log.debug(f"SEI drop: buffering_period SPS id={bp_id} khong hop le")
                    keep = False
            except Exception:
                keep = False

        if keep:
            out_payloads += _encode_ff(ptype) + _encode_ff(psize) + payload_raw
        else:
            dropped += 1
...
class AccessUnitBuffer:
    ...
    def _total_bytes(self) -> int:
        return sum(len(nd) for nds in self._buf.values() for nd in nds)

    def flush(self):
        if not self._buf: return

        # Drop AU qua nho: tranh "missing picture in access unit with size N"
        # Chi flush neu co it nhat 1 slice NAL hop le
        has_slice = any(nt in SLICE_TYPES for nt in self._buf)
        total     = self._total_bytes()
        if not has_slice or total < MIN_AU_BYTES:
            log.debug(f"AU skip: has_slice={has_slice}, total_bytes={total} < {MIN_AU_BYTES}")
            self._buf.clear(); self._has_slice = False; self._skipped_au += 1
            return
    ...
    @property
    def skipped_au(self): return self._skipped_au
...
def scan_bad_file(
    ...
    stats = dict(
        ...
        bad_slice=0, bad_escape=0, bad_nal0=0, access_units=0, skipped_au=0,
    )
    ...
    with open(bad_file,"rb") as fin, \
        ...
        def accept_nal(nt: int, nd: bytes):
            ...
            # NAL type 0 = undefined / garbage -> luon reject
            # Log ffplay: "Invalid NAL unit 0, skipping."
            if nt == 0:
                stats["bad_nal0"] += 1; return
        ...
        stats["skipped_au"]   = au_buf.skipped_au
...
def main():
    ...
    try:
        ...
        if os.path.exists(args.output):
            ...
            log.info(f"   Bo NAL-0 rac : {stats['bad_nal0']}")
            log.info(f"   Bo AU nho    : {stats['skipped_au']}")
